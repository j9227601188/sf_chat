...
// TODO надо ещё:
// 1 класс для хранения сообщений - сделано;
// 2 класс сообщений - сделано в виде структуры;
// 3 класс Shared_ptr;
...
void Hello(Chat* chat, StoreUsers* su) {
	...
	while (true)
	{
		...
		case '1':
			...
			if (su->checkLogin(name, pass)) {			// если такой юзер(логин/пароль) есть в базе
				if (!chat->UserInChat(name)) {			// проверка что уже в чате!!!
					new User(chat, name);
					name.clear();
					pass.clear();
				...
				else cout << "Юзер " << name << " уже в чате." << endl;
			}
